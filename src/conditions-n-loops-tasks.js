...
function canQueenCaptureKing(queen, king) {
  ...
  const dig =
    (queen.x - king.x) * (queen.x - king.x) ===
    (queen.y - king.y) * (queen.y - king.y);
...
function convertNumberToString(numberStr) {
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    ...
    if (word) {
      newNumber += word;
      if (i < numberStr.length - 1) newNumber += ' ';
    }
  }
 ...
 *
 * @example:
 *   => 2    => 1 + 2 === 3 + 0 then balance element is 5 and its index = 2
 *  [2, 3, 9, 5] => 2       => 2 + 3 === 5 then balance element is 9 and its index = 2
 *   => -1   => no balance element
 */
